...
return{
 ...
 fetch('https://mymentor-server.herokuapp.com/contact/edit', {
  ...
  .then((json) => {
    fetch("https://lyf-styl-reservation.herokuapp.com/users/" + this.id, {
        method: "PUT",
        body: JSON.stringify({
          name: this.name,
    email:this.email,
  ...
  // when student deletes account
   async  deleteMail(){
    this.loading = true
    try{
 fetch('"https://lyf-styl-reservation.herokuapp.com/user/delete', {
  method: 'POST',
  body: JSON.stringify({
    name: this.name,
    email:this.email
  ...
  .then((json) => {
     this.loading = false
          //  delete profile
        fetch('"https://lyf-styl-reservation.herokuapp.com/user'+ this.id ,{
          method: 'DELETE',
          headers: {
          "Content-type": "application/json; charset=UTF-8",
          Authorization: `Bearer ${localStorage.getItem("jwt")}`,
